...
scale_factor_for_poverty_lines = np.genfromtxt('scale_factor_WB_HH_toPOVCAL2019.csv', dtype = str, delimiter=',');
...
price_blanket = np.repeat(150,14)
income_elasticities_USA  = income_elasticities[1232-14:1232]
price_differentiated = np.multiply(np.expand_dims(price_blanket,axis=1),income_elasticities_USA)*1.15908 ### normalization constant for the USA


fig, ax = plt.subplots()
y_pos = np.arange(len(category_names))
ax.barh(y_pos, price_blanket, align='center', label = 'blanket')
...
ax.set_xlabel('Carbon price $/ton');
...
price_blanket_CHINA = np.repeat(50,14)
income_elasticities_CHINA = income_elasticities[14*12-14:12*14]
price_differentiated_CHINA = np.multiply(np.expand_dims(price_blanket_CHINA,axis=1),income_elasticities_CHINA)*1.02232
 ### normalization constant for China


fig, ax = plt.subplots()
y_pos = np.arange(len(category_names))
ax.barh(y_pos, price_blanket_CHINA, align='center', label = 'blanket')
...
price_blanket_SA = np.repeat(50,14)
income_elasticities_SA = income_elasticities[14*48-14:48*14]
price_differentiated_SA = np.multiply(np.expand_dims(price_blanket_SA,axis=1),income_elasticities_SA)*1.05383



fig, ax = plt.subplots()
y_pos = np.arange(len(category_names))
ax.barh(y_pos, price_blanket_SA, align='center', label = 'blanket')
...
price_blanket = np.repeat(150,14)
income_elasticities_USA  = income_elasticities[1232-14:1232]
price_differentiated = np.multiply(np.expand_dims(price_blanket,axis=1),income_elasticities_USA**3)*0.938024 ### normalization constant for the USA
...
price_blanket_CHINA = np.repeat(50,14)
income_elasticities_CHINA = income_elasticities[14*12-14:12*14]
price_differentiated_CHINA = np.multiply(np.expand_dims(price_blanket_CHINA,axis=1),income_elasticities_CHINA**3)*0.973105 ## cubed normalization constant
 ### normalization constant for China
...
price_blanket_SA = np.repeat(50,14)
income_elasticities_SA = income_elasticities[14*48-14:48*14]
price_differentiated_SA = np.multiply(np.expand_dims(price_blanket_SA,axis=1),income_elasticities_SA**3)*0.620955
...
price_differentiated = np.multiply(np.expand_dims(price_blanket,axis=1),income_elasticities_USA)*1.15908 ### normalization constant for the USA
y_pos = np.arange(len(category_names))
ax2.barh(y_pos, price_blanket, align='center', label = 'blanket')
...
ax2.set_xlabel('Carbon price $/ton');
...
ax3.set_ylabel("CO2e tons/capita");
...
#########################################################################
#####BLANKET SCENARIO PREPARATION carbon price = 0.05$/kg is 50 $/ton####
#########################################################################
...
##### make price array with blanket prices ######
...
embodied_costs_per_country_blanket = np.zeros((88,1));
embodied_costs_per_country_luxury_pre = np.zeros((88,1));
for i in range(1,89):
    embodied_costs_per_country_blanket[i-1] = scen1_df_embodied_carbon_costs_pc.multiply(pop_percentile_2019).sum(axis=1).iloc[14*i-14:i*14].sum()
...
normalization_constant_scen_luxury_to_blanket = embodied_costs_per_country_blanket/embodied_costs_per_country_luxury_pre

#####Luxury SCENARIO carbon price is differentiated per product with income elasticity of demand
###### once with normalization (i.e. carbon costs in society same as in blanket scenario, once without)
...
for i in range(1,89):
     scen2_2_df_embodied_carbon_costs_pc.iloc[i*14-14:i*14] = scen2_df_embodied_carbon_costs_pc_pre_normalization.iloc[i*14-14:i*14]*float(normalization_constant_scen_luxury_to_blanket[i-1]);   
...
for k in range(1,N+1):    
            ...
            USA_total_abated_carbon_SCEN1 = (sum(USA_pre_tax_carbon_cons_total.sum(axis = 1))-sum(USA_post_tax_carbon_cons_total_SCEN1.sum(axis = 1)))/(10**12) #### unit in gigaton
            USA_total_abated_carbon_SCEN2_2 = (sum(USA_pre_tax_carbon_cons_total.sum(axis = 1))-sum(USA_post_tax_carbon_cons_total_SCEN2_2.sum(axis = 1)))/(10**12) #### unit in gigaton
            USA_abated_carbon_per_category_SCEN1 = (USA_pre_tax_carbon_cons_total.sum(axis = 1) - USA_post_tax_carbon_cons_total_SCEN1.sum(axis = 1))/(10**12) #### gigaton unit
            ...
            USA_abated_carbon_per_category_SCEN2_2 = (USA_pre_tax_carbon_cons_total.sum(axis = 1) - USA_post_tax_carbon_cons_total_SCEN2_2.sum(axis = 1))/(10**12) #### gigaton unit
            ...
            SA_total_abated_carbon_SCEN1 = (sum(SA_pre_tax_carbon_cons_total.sum(axis = 1))-sum(SA_post_tax_carbon_cons_total_SCEN1.sum(axis = 1)))/(10**12) #### unit in gigaton
            SA_total_abated_carbon_SCEN2_2 = (sum(SA_pre_tax_carbon_cons_total.sum(axis = 1))-sum(SA_post_tax_carbon_cons_total_SCEN2_2.sum(axis = 1)))/(10**12) #### unit in gigaton
            SA_abated_carbon_per_category_SCEN1 = (SA_pre_tax_carbon_cons_total.sum(axis = 1) - SA_post_tax_carbon_cons_total_SCEN1.sum(axis = 1))/(10**12) #### gigaton unit
            SA_abated_carbon_per_category_share_SCEN1_simruns[:,k-1] = SA_abated_carbon_per_category_SCEN1/SA_total_abated_carbon_SCEN1              
            SA_abated_carbon_per_category_SCEN2_2 = (SA_pre_tax_carbon_cons_total.sum(axis = 1) - SA_post_tax_carbon_cons_total_SCEN2_2.sum(axis = 1))/(10**12) #### gigaton unit
            ...
            CHINA_total_abated_carbon_SCEN1 = (sum(CHINA_pre_tax_carbon_cons_total.sum(axis = 1))-sum(CHINA_post_tax_carbon_cons_total_SCEN1.sum(axis = 1)))/(10**12) #### unit in gigaton
            CHINA_total_abated_carbon_SCEN2_2 = (sum(CHINA_pre_tax_carbon_cons_total.sum(axis = 1))-sum(CHINA_post_tax_carbon_cons_total_SCEN2_2.sum(axis = 1)))/(10**12) #### unit in gigaton        
            CHINA_abated_carbon_per_category_SCEN1 = (CHINA_pre_tax_carbon_cons_total.sum(axis = 1) - CHINA_post_tax_carbon_cons_total_SCEN1.sum(axis = 1))/(10**12) #### gigaton unit
            ...
            CHINA_abated_carbon_per_category_SCEN2_2 = (CHINA_pre_tax_carbon_cons_total.sum(axis = 1) - CHINA_post_tax_carbon_cons_total_SCEN2_2.sum(axis = 1))/(10**12) #### gigaton unit
...
price_blanket = np.repeat(150,14)
income_elasticities_USA  = income_elasticities[1232-14:1232]
price_differentiated = np.multiply(np.expand_dims(price_blanket,axis=1),income_elasticities_USA**exponent_parameter)*normalization_constant_scen_luxury_to_blanket[87] ### normalization constant for the USA
...
price_blanket_CHINA = np.repeat(50,14)
income_elasticities_CHINA = income_elasticities[14*12-14:12*14]
price_differentiated_CHINA = np.multiply(np.expand_dims(price_blanket_CHINA,axis=1),income_elasticities_CHINA**exponent_parameter)*normalization_constant_scen_luxury_to_blanket[11]
 ### normalization constant for China
...
ax1.annotate("Blanket tax", (0.14, 11), (0.14, 11));
...
ax1.set_ylabel('% reducton CO2e through luxury tax')
...
ax1.set_ylabel('CO2e tons/capita')
...
ax2.plot(percentiles, USA_per_cent_reduction_emissions_pc_SCEN1_mean, color = color, label = "reduction blanket tax")
...
LABELS = ['blanket', 'luxury']
df_US_bar = pd.concat([USA_frame1, USA_frame2], ignore_index = True)

df_US_bar = df_US_bar.rename(index={df_US_bar.index[0]: 'blanket', df_US_bar.index[1]: ' luxury'})
...
ax2.plot(percentiles, CHINA_per_cent_reduction_emissions_pc_SCEN1_mean, color = color, label = "reduction blanket tax")
ax2.tick_params(axis='y', labelcolor=color)
ax2.set_ylim((4,6))
...
df_CHINA_bar = df_CHINA_bar.rename(index={df_CHINA_bar.index[0]: 'blanket', df_CHINA_bar.index[1]: ' luxury'})
...
ax2.plot(percentiles, SA_per_cent_reduction_emissions_pc_SCEN1_mean, color = color, label = "reduction blanket tax")
ax2.tick_params(axis='y', labelcolor=color)
ax2.set_ylim((3,10))
ax2.set_yticks([4,4.5,5,5.5,6])
ax1.legend(bbox_to_anchor=(0.2, 0.0, 0.5, 0.5), frameon = False)
...
df_SA_bar = df_SA_bar.rename(index={df_SA_bar.index[0]: 'blanket', df_SA_bar.index[1]: ' luxury'})
...
ax1.set_xlabel('1st - 100th blanket tax')
...
ax1.set_xlim(-8,5);
ax1.margins(x=0,y=0);
ax1.annotate('progressive blanket',xy= (-5,7), xytext =(-5,7), fontsize = 8)
ax1.annotate('regressive luxury',xy= (-6,5), xytext =(-5,5), fontsize = 8)
ax1.annotate('progressive blanket',xy= (-5,-20), xytext =(-5,-20), fontsize = 8)
ax1.annotate('progressive luxury',xy= (-5,-22), xytext =(-5,-22), fontsize = 8)
ax1.annotate('regressive blanket',xy= (1,-20), xytext =(1,-20), fontsize = 8)
ax1.annotate('progressive luxury',xy= (1,-22), xytext =(1,-22), fontsize = 8)
ax1.annotate('regressive blanket',xy= (1,7), xytext =(1,7), fontsize = 8)
...
ax2.set_xlabel('1st - 100th blanket tax')
ax2.set_ylabel('1st - 100th percentile Luxury tax')
ax2.set_xlim(-8,5);
ax2.margins(x=0,y=0);
ax2.annotate('progressive blanket',xy= (-5,7), xytext =(-5,7), fontsize = 8)
ax2.annotate('regressive luxury',xy= (-6,5), xytext =(-5,5), fontsize = 8)
ax2.annotate('progressive blanket',xy= (-5,-20), xytext =(-5,-20), fontsize = 8)
ax2.annotate('progressive luxury',xy= (-5,-22), xytext =(-5,-22), fontsize = 8)
ax2.annotate('regressive blanket',xy= (1,-20), xytext =(1,-20), fontsize = 8)
ax2.annotate('progressive luxury',xy= (1,-22), xytext =(1,-22), fontsize = 8)
ax2.annotate('regressive blanket',xy= (1,7), xytext =(1,7), fontsize = 8)
...
ax3.set_ylabel('CO2e tons/capita', fontsize = 8)
...
ax7.plot(percentiles, USA_per_cent_reduction_emissions_pc_SCEN1_mean, color = "tab:blue", label = "reduction blanket tax")
...
ax4.set_ylabel('CO2e tons/capita', fontsize = 8)
...
ax8.plot(percentiles, CHINA_per_cent_reduction_emissions_pc_SCEN1_mean, color = "tab:blue", label = "reduction blanket tax")
...
for i in range(0,14):
     ...
     ax5.set_xticks([1,2])
     ax5.set_xticklabels(["blanket", "luxury"], fontsize = 8)
...
for i in range(0,14):
     ...
     ax6.set_xticks([1,2])
     ax6.set_xticklabels(["blanket", "luxury"], fontsize = 8)
...
ax2.plot([-8,5], [0,0], color = "black", linestyle = "--")
ax2.plot([-25,5], [-25,5], color = "black", label = " x = y")
ax2.set_xlabel(r'1st $-$ 100th percentile blanket tax')
ax2.set_ylabel(r'1st $-$ 100th percentile luxury tax')
ax2.set_xlim(-7,5);
ax2.set_ylim(-18,1);
ax2.margins(x=0,y=0);
#ax2.annotate('progressive blanket',xy= (-5,7), xytext =(-5,7), fontsize = 9)
#ax2.annotate('regressive luxury',xy= (-6,5), xytext =(-5,5), fontsize = 9)
ax2.annotate('progressive blanket',xy= (-6,-16), xytext =(-6,-16), fontsize = 9)
ax2.annotate('progressive luxury',xy= (-6,-17), xytext =(-6,-17), fontsize = 9)
ax2.annotate('regressive blanket',xy= (1,-16), xytext =(1,-16), fontsize = 9)
ax2.annotate('progressive luxury',xy= (1,-17), xytext =(1,-17), fontsize = 9)
#ax2.annotate('regressive blanket',xy= (1,7), xytext =(1,7), fontsize = 9)
...
ax8.plot(percentiles, SA_per_cent_reduction_emissions_pc_SCEN1_mean, color = "tab:blue", label = "reduction blanket tax")
ax8.tick_params(axis='y', labelcolor=color)
#ax4.legend(bbox_to_anchor=(0.2, 0.01, 0.5, 0.5), frameon = False)
#ax8.legend(bbox_to_anchor=(0.2, 0.08, 0.5, 0.5), frameon = False)
...
ax1.set_ylabel('CO2e tons/capita', fontsize = 8)
ax1.tick_params(axis='y')
...
ax3.plot(percentiles, USA_per_cent_reduction_emissions_pc_SCEN1_mean, color = "tab:blue", label = "reduction blanket tax")
...
ax2.set_ylabel('CO2e tons/capita', fontsize = 8)
...
ax4.plot(percentiles, SA_per_cent_reduction_emissions_pc_SCEN1_mean, color = "tab:blue", label = "reduction blanket tax")
...
ax5.plot(percentiles, USA_per_cent_reduction_emissions_pc_SCEN1_mean, color = "tab:blue", label = "reduction blanket tax")
...
ax6.plot(percentiles, SA_per_cent_reduction_emissions_pc_SCEN1_mean, color = "tab:blue", label = "reduction blanket tax")
...
price_blanket = np.repeat(150,14)
income_elasticities_USA  = income_elasticities[1232-14:1232]
price_differentiated_cubed = np.multiply(np.expand_dims(price_blanket,axis=1),income_elasticities_USA**3)*0.938024 ### normalization constant for the USA
price_differentiated = np.multiply(np.expand_dims(price_blanket,axis=1),income_elasticities_USA)*1.15908 

y_pos = np.arange(len(category_names))
ax3.barh(y_pos, price_blanket, align='center', label = 'blanket')
...
ax3.set_xlabel('Carbon price $/ton');
...
income_elasticities_SA = income_elasticities[14*48-14:48*14]
price_differentiated_SA = np.multiply(np.expand_dims(price_blanket_SA,axis=1),income_elasticities_SA)*1.05383
price_differentiated_SA_cubed = np.multiply(np.expand_dims(price_blanket_SA,axis=1),income_elasticities_SA**3)*0.620955

y_pos = np.arange(len(category_names))
ax4.barh(y_pos, price_blanket_SA, align='center', label = 'blanket')
...
ax4.set_xlabel('Carbon price $/ton');
...
fig3_a_b = np.genfromtxt('data_fig3_a_b.csv', dtype = str, delimiter=',')
plot_data_trade_off = np.genfromtxt('plot_data_trade_off.csv', dtype = str, delimiter=',').astype(float)
...
ax1.plot(percentiles, fig3_a_b[1:102,0].astype(float)*100, label = "redistribution")
ax1.plot(percentiles, fig3_a_b[1:102,1].astype(float)*100, label = "retrofit")
...
ax2.plot(percentiles, fig3_a_b[1:102,2].astype(float)*100, label = "redistribution")
ax2.plot(percentiles, fig3_a_b[1:102,3].astype(float)*100, label = "retrofit")
...
####  test how much is the total global tax revenue compared to total global consumption in the luxury scenario and blanket scenario
...
df_yearly_emissions_total_2019_post_blanket_tax = pd.DataFrame(columns = labels2, index = meta_data_countries[:,0])
for i in range(1,89):  
     for j in range(0,100):
            ...
            df_yearly_emissions_total_2019_post_blanket_tax.iloc[i-1,j]= scen1_average_emissions_granular_total.iloc[i*14-14:i*14,j].sum();
            
mega_test = df_yearly_emissions_total_2019_post_luxury_tax.stack()
mega_test2 = df_yearly_emissions_total_2019_post_blanket_tax.stack()
...
national_gini_post_blanket = np.zeros((88,1))
...
for i in range(1,89):
    ...
    national_gini_post_blanket[i-1,0] = gini_array_version(np.squeeze(stacked_population_percentiles[100*i-100:100*i]), np.array(df_yearly_emissions_total_2019_post_blanket_tax.stack().iloc[100*i-100:100*i]))
